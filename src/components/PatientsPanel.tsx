...
import { ChevronDown, ChevronRight, Loader2, Users, UserPlus, Pencil, Trash2, Check, X, Search, CalendarDays, FileText, Download, HeartPulse, Sparkles } from 'lucide-react';
      ...
      <div className="relative overflow-hidden rounded-[2rem] border border-white/80 bg-[#fffdf8] shadow-[0_20px_60px_-30px_rgba(16,87,62,0.35)]">
        ...
        <div className="relative border-b border-emerald-100/80 bg-gradient-to-r from-emerald-50/80 via-white to-amber-50/70 px-6 py-7 sm:px-8">
          ...
          <div>
            <p className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.28em] text-emerald-700">
              <Sparkles size={13} /> Tu consultorio
            </p>
            <h2 className="mt-2 flex items-center gap-2 font-serif text-3xl font-semibold tracking-tight text-slate-800">
              <Users size={26} className="text-emerald-600" strokeWidth={1.8} /> Mis pacientes
            </h2>
            <p className="mt-2 max-w-xl text-sm leading-6 text-slate-500">Conoce el historial de cada persona, consulta sus planes y mantén sus datos siempre al día.</p>
          </div>
          <div className="flex items-center gap-2 rounded-2xl border border-white bg-white/80 px-4 py-3 shadow-sm">
            <span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-100 text-emerald-700"><HeartPulse size={18} /></span>
            <span><strong className="block text-lg leading-none text-slate-800">{patients.length}</strong><small className="text-[11px] text-slate-500">pacientes registrados</small></span>
          </div>
          </div>
          <button
            type="button"
            onClick={onNewPatient}
            className="flex items-center gap-2 rounded-full bg-emerald-600 px-5 py-3 text-xs font-semibold text-white shadow-lg shadow-emerald-700/15 transition hover:-translate-y-0.5 hover:bg-emerald-700 hover:shadow-md"
          >
        ...
        {patients.length === 0 ? (
          <div className="flex flex-col items-center py-12 text-center">
            <span className="grid h-16 w-16 place-items-center rounded-2xl bg-emerald-50 text-emerald-500"><Users size={28} /></span>
            <p className="mt-4 text-sm font-medium text-slate-700">Tu lista todavía está vacía</p>
            <p className="mt-1 max-w-sm text-sm leading-6 text-slate-400">Registra un paciente para comenzar a crear valoraciones y planes personalizados.</p>
          </div>
        ) : filtered.length === 0 ? (
          <p className="py-12 text-center text-sm text-slate-400">No se encontró ningún paciente con “{query}”.</p>
        ) : (
          <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white/75 divide-y divide-slate-100">
